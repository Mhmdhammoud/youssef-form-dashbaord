...
const Index = () => {
  ...
  const [submitUpdateOrderSerial, { loading: UpdateLoading }] =
    useUpdateOrderMutation({
      variables: {
        _id: order?._id!,
        input: {
          ...
          product: {
            left: {
              ...BTEOrder.product.left,
              serialNumber: BTEOrder?.product?.left?.serialNumber,
            },
            right: {
              ...BTEOrder.product.right,
              serialNumber: BTEOrder?.product?.right?.serialNumber,
            },
          },
        },
      },
  ...
  const handleSubmitSerialNumber = useCallback(() => {
    submitUpdateOrderSerial()
      .then((res) => {
        if (
          res?.data?.updateOrder?.errors &&
          res?.data?.updateOrder?.errors.length > 0
        ) {
          setNotificationToast({
            message: res?.data?.updateOrder?.errors[0].message,
            title: 'Error',
          })
          setNotificationOpen(true)
        } else {
          ...
          setNotificationToast({
            message: 'Serial number has been updated successfully',
            title: 'Success',
          })
          setNotificationOpen(true)
          refetch()
  ...
  }, [refetch, submitUpdateOrderSerial])
      ...
      if (order?.company?.country === 'KW') {
        if (order?.deliveryDetails.urgent) {
          if (moment(order?.createdAt).day() === 4) {
            return moment(order?.createdAt).add(2, 'days').format('DD-MM-YYYY')
          } else {
            return moment(order?.createdAt).add(1, 'days').format('DD-MM-YYYY')
          }
        } else {
          if (moment(order?.createdAt).day() - 3 <= 1) {
            return moment(order?.createdAt).add(4, 'days').format('DD-MM-YYYY')
          } else {
            return moment(order?.createdAt).add(3, 'days').format('DD-MM-YYYY')
          }
        }
      } else {
        if (order?.deliveryDetails.urgent) {
  ...
  return (
    <React.Fragment>
      ...
      <Wrapper
        ...
        classes="mb-4 print:portrait:my-[-52px]"
      >
                  ...
                  {order?.logs && order?.logs.length > 0 && (
                    <div className="flex justify-end mt-3 space-x-3 print:hidden">
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-md text-white bg-amber-500 hover:bg-white-700"
                      >
                        Edit
        ...
        />
        <EditSerialModal
          order={BTEOrder as Order}
          open={editOrderOpen}
          setOpen={setEditOrderOpen}
          handleChange={handleChangeSerialNumber}
